...
import { runTeamModeSeasonSimulation, runPlayerModeSeasonSimulation, type TeamSeasonResult } from "./api/SeasonSimulation.ts";
...
function App() {
	...
	const handleRunSimulation = async () => {
		...
		try {
			...
			if (mode === "Team") {
				const method = teamStat === 'pythagorean' ? 'Pythagorean' : 'Elo';
				results = await runTeamModeSeasonSimulation(method);
			} else if (mode === "Player") {
				results = await runPlayerModeSeasonSimulation(hitterStat, pitcherStat, transactions, setTransactions);
			}
